    ...
    def acc(self, ep):
        ...
        with self.model.graph.as_default():
            while(ind < n):
                ...
                # print(sentences)
                # sentences = ["On this occasion, wounded pride exasperated her
                # wrath still _____."]
                ...
                for i in range(words.shape[0]):
                    ...
                    # a = self.sess.run(self.model.accuracy, feed_dict = {self.model.words :
                    # wordsprim, self.model.sents : sentencesprim})
                    cur_ac = 0
                    fst = 0
                    # print(output[0:5])
                    # print(output.shape)
    ...
    def test(self):
        ...
        with self.model.graph.as_default():
            while(ind < n):
                ...
                words1 = self.test_data[ind:int((ind+j)/2),1]
                words2 = self.test_data[int((ind+j)/2):j, 1]

                # sents = self.test_data[ind:ind+1,0]
                # words = self.test_data[ind:ind+1,1]
                t = self.sess.run(self.model.output, feed_dict={self.model.words[0]: words1, self.model.words[1]: words2, self.model.sents[0]: sentences1, self.model.sents[1]: sentences2})
